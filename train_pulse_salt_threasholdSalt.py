...
from utils import *
from add_noise import addPulseNoise_dncnn as addPulseNoise
...
parser.add_argument("--threasholdVal", type=int, default=36, help="在训练集设置时用于判断当前像素是否为impulse噪声点,要稍微比SVM的threashold大一些，解决漏判问题")
        ...
        for k in range(len(dataset_val)):
            ...
            if opt.mode == 'S':
                if (opt.val_noiseL > 2):
                    ...
                    imgn_val = addPulseNoise(img_val, opt.val_noiseL);  # 生成特定强度的脉冲噪声
            else:
                if (opt.val_noiseL > 2):
            ...
            if epoch==0:
                ...
                imwrite(savePath+"outImage/imagn"+"/Image_"+str(k+1)+"noise.png",origin);
