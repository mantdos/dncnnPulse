...
parser.add_argument("--dncnnDir", type=str, default="./result/dncnnPulseNoiseResult_spec_ThreasholdTest/", help='dncnn模型所在文件的路径前缀')
parser.add_argument("--svmDir", type=str, default="./svmResult_threasholdTest/", help='SVM模型所在文件的路径前缀')
parser.add_argument("--saveDir", type=str, default="./result/dncnn_svm_threasholdTest/", help='测试结果统一保存位置前缀')
...
def dncnnDeal(save_dir,img,imgn,dncnnModel,noise,i,suffix,logger):
    ...
    saveDir = save_dir+"/dncnnOutPutResult/";
...
def testThreeSituation(noiseList,threasholdList):
    ...
    for noise in noiseList:
        for threashold in threasholdList:
            ...
            save_dir = opt.saveDir + "noise_" + str(noise) +"_threashold_"+str(threashold) +"/";
            # 注册logger文件
            logger_name = "result_"+str(noise)+"_threashold_"+str(threashold);
            logger_dir = save_dir;
            ...
            # 修改加载的训练集名称
            opt.trainDataFileName = "threashold_"+str(threashold)+"_100000_10_60_yes.h5";
            getTrainData(logger);
            logger.info("dncnn层数："+str(opt.num_of_layers));
            # 根据噪声强度和threashold加载threasholdSalt的模型，即2.3即可
            dncnn_threasholdSalt_dir = opt.dncnnDir + "threashold_"+str(threashold)+ "&noise_"+str(noise+2);
            ...
            # 根据噪声强度获取SVM模型
            svmDir = opt.svmDir+"threashold_"+str(threashold)+"/model_"+str(noise)+"/";
            ...
            for i in ls:
                ...
                # dncnn_threasholdSalt+svm处理的随机脉冲噪声
                logger.info("对比dncnnThreasholdSalt+SVM处理后噪声降噪效果");
                averPsnrList_dncnnThreasholdSalt.append(
                    dncnnDeal(save_dir,img, imgn_svm, dncnnModel_threasholdSalt, noise, i, "dncnnThreasholdSalt", logger));
            logger.info("***********************************************************************");
...
if __name__ == '__main__':
    ...
    if not os.path.exists("./result/"):
        os.mkdir("./result/");
    testThreeSituation([0.3],[5,10,20,30,40]);