...
# noise<1代表使用脉冲噪声，2>noise>1并且代表使用的盐噪声，noise>2代表是使用的经过threshhold处理过的盐噪声  noise>3代表与SVM联合训练的dncnn
...
parser.add_argument("--threasholdVal", type=int, default=10, help="在训练集设置时用于判断当前像素是否为impulse噪声点,要稍微比SVM的threashold大一些，解决漏判问题")
...
def main(f,savePath,depth,noiseS):
    ...
    logger.info('*********opt.threasholdVal={:2d}'.format(opt.threasholdVal));
    #
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            if opt.mode == 'S':
                if (opt.noiseL > 3):
                    ...
                    imgn_train = addSaltNoise(img_train, opt.noiseL-1);   # 生成特定强度的脉冲噪声
                else:
...
def realmain():
    ...
    depthList = [17];
    noiseList = [3.1];
    # threasholdList = [5,10,20,30];_ThreasholdTest
