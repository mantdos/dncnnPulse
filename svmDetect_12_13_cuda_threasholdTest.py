...
parser.add_argument("--saveDir", type=str, default="./svmResult_threasholdTest/", help='SVM的整体保存路径')
...
def main(svmModelSaveDir,noise,cnt):
    opt.noiseSpec = noise;
    logger_name = "modelAccuracy_noise_"+str(noise)+"threashold_"+str(threasholdVal);
    if not os.path.exists(opt.saveDir):
        os.mkdir(opt.saveDir);
    if opt.noiseSpec<0:
...
if __name__ == '__main__':
    ...
    if opt.isTestMode:
        ...
        if not os.path.exists(opt.saveDir):
            os.mkdir(opt.saveDir);
        for threasholdVal in [5,10,15,20,25,30,35,40]:
            opt.threasholdVal = threasholdVal;
            opt.createNewDataFileName = "threashold_" + str(threasholdVal) + "_" + DataFileName;
            opt.trainDataFileName = opt.createNewDataFileName;
            trainDataInit(opt.trainImg, opt.trainSetSize, opt.sampleNumPerImg, opt.noiseMin, opt.noiseMax,
                          opt.noiseSpec, opt.trainDataSaveDir, opt.createNewDataFileName);
            for noise in [0.3]:
