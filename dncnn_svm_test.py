...
def svmDeal(imgn,svmModel,logger):
    #在使用SVM处理之前需要先得到特征维度和归一化参数等
    logger.info("加载训练集数据获取归一化参数");
    getTrainData(logger);
    # 对imgn中所有点计算特征,保存在data中，data：size*size*?
...
def testThreeSituation(noiseList):
    ...
    for noise in noiseList:
        ...
        # 根据噪声强度获取SVM模型
        svmDir = opt.svmDir+str(noise)+"/SVM.model";
        logger.info("加载SVM模型："+svmDir);
        ...
        logger.info("原始dncnn PSNR平均值：{:.3f}".format(averPsnrList_dncnnOrigin.sum()/12));
        logger.info("dncnnSalt处理结果："+str(averPsnrList_dncnnSalt));
        logger.info("dncnnSalt PSNR平均值：{:.3f}".format(averPsnrList_dncnnSalt.sum()/12));
        logger.info("dncnnThreasholdSalt处理结果："+str(averPsnrList_dncnnThreasholdSalt));
        logger.info("dncnnThreasholdSalt PSNR平均值：{:.3f}".format(averPsnrList_dncnnThreasholdSalt.sum()/12));

if __name__ == '__main__':
    envpath = '/home/p/anaconda3/lib/python3.8/site-packages/cv2/qt/plugins/platforms'
    os.environ['QT_QPA_PLATFORM_PLUGIN_PATH'] = envpath
    testThreeSituation([0.1]);